...
void Reflection::onPluginLoad()
{
	...
	g.glBindTexture(GL_TEXTURE_2D, textureId[1]);
	g.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	g.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	g.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
	g.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
...
bool Reflection::paintGL()
{
	...
	reflection1.translate(2*scene()->boundingBox().min().x(), 0, 0);
    ...
    reflection2.scale(1,1,1);
	reflection2.translate(0, 0, 2*scene()->boundingBox().min().z());
    ...
    g.glBindTexture(GL_TEXTURE_2D, textureId[1]);
    Point V4 = Point(b.min().x(), b.max().y(), b.min().z());
    Point V5 = Point(b.min().x(), b.max().y(), b.max().z());
    drawRect(g, V0, V4, V5, V3);
    
    g.glBindTexture(GL_TEXTURE_2D, textureId[2]);
    Point V6 = Point(b.max().x(), b.max().y(), b.min().z());
    drawRect(g, V0, V1, V6, V4);
